...
Both share the same container image (ECR), IAM execution role, and binary. Both use `lambda.Start()` — `HANDLER_MODE` env var selects the handler: API returns `LambdaFunctionURLStreamingResponse` (streaming); Worker returns standard JSON responses.
