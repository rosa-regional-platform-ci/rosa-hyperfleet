#!/usr/bin/env python3
"""
desire_timing_test_sns.py

Same as desire_timing_test.py but also publishes SNS notifications to the
RC's specs-notifications topic after each desire write, mimicking what the
hyperfleet-operator does via snspublisher.Publisher.

SNS topic ARN pattern (from snspublisher/publisher.go):
  arn:aws:sns:<region>:<rc_account_id>:<mc_cluster_name>-specs-notifications

Message payload (SpecNotification struct):
  {"documentID": "<id>", "tableSuffix": "-applydesires" | "-readdesires"}

The RC account ID is resolved automatically via sts:GetCallerIdentity using
the current AWS profile, or can be passed explicitly with --rc-account-id.

Flow:
  1. Parse .ephemeral-envs, pick a ready env (or use --env-id)
  2. Resolve RC account ID via STS (or --rc-account-id)
  3. Derive DynamoDB table names: eph-{id}-mc01-{specs|status}-{applydesires|readdesires}
  4. Write ApplyDesire (ServerSideApply, ConfigMap) → publish SNS notification
  5. Write ReadDesire for the same ConfigMap  → publish SNS notification
  6. Poll status-applydesires until Successful=True  -> log timing
  7. Poll status-readdesires until populated         -> log timing
  8. Flip ApplyDesire to Type=Delete → publish SNS notification
  9. Poll status-applydesires until delete Successful=True -> log timing
 10. Delete ApplyDesire and ReadDesire from specs tables
 11. Print full timing summary

Usage:
  python3 desire_timing_test_sns.py [--env-id <id>] [--mc mc01]
                                    [--profile <aws-profile>]
                                    [--rc-account-id <12-digit-id>]
...
def parse_args():
    ...
    p.add_argument(
        "--rc-account-id",
        help="RC AWS account ID for SNS topic ARN (default: resolved via sts:GetCallerIdentity)",
    )
    p.add_argument(
...
def publish_spec_notification(
    sns_client, topic_arn: str, document_id: str, table_suffix: str
):
    """
    Publish a SpecNotification to SNS, mimicking snspublisher.Publisher.Publish()
    in hyperfleet-operator/internal/dynamo/snspublisher/publisher.go.

    Payload: {"documentID": "<id>", "tableSuffix": "-applydesires"|"-readdesires"}
    """
    payload = json.dumps({"documentID": document_id, "tableSuffix": table_suffix})
    resp = sns_client.publish(TopicArn=topic_arn, Message=payload)
...
def main():
    ...
    msg_id = publish_spec_notification(
        sns_client, sns_topic_arn, apply_doc_id, "-applydesires"
    )
    ok(f"SNS notification sent (MessageId={msg_id})")
    ...
    msg_id = publish_spec_notification(
        sns_client, sns_topic_arn, read_doc_id, "-readdesires"
    )
    ok(f"SNS notification sent (MessageId={msg_id})")
    print()
