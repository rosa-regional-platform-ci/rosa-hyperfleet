...
| `ami_kms_key_arn`               | ARN of the Red Hat KMS key encrypting FIPS AMI EBS snapshots. When set, adds `kms:CreateGrant` and `kms:DescribeKey` to the Karpenter controller role. | `string`       | `""`                                                    | no       |
