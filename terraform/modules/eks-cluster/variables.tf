...
variable "ami_kms_key_arn" {
  description = "ARN of a KMS key used to encrypt a custom AMI's EBS snapshots. When set, an IAM policy granting kms:CreateGrant and kms:DescribeKey on this key is added to the Karpenter controller role only. Not used by the current standard Bottlerocket AMI; leave empty to skip KMS policy creation."
  type        = string
  default     = ""
}
