...
resource "aws_iam_role_policy" "zoa_aws_write" {
...
#   - EventBridge "reaper" (5min): terminate expired rosa-boundary containers
#   - Async self-invoke: execute approved TAs (fan-out from reconciler)
#
# Code-level deadlines (env var tunable without code change):
#   - Scheduled routes (reconciler/gc/reaper): RECONCILER_DEADLINE_SECONDS (55s)
...
resource "aws_lambda_function" "worker" {
  ...
  description   = "ZOA Worker for ${var.cluster_id} - reconciler, GC, reaper, TA execution"
  role          = aws_iam_role.lambda.arn
  package_type  = "Image"
  image_uri     = var.lambda_image_uri
  architectures = ["x86_64"]
...
resource "aws_scheduler_schedule" "reaper" {
  ...
  description = "Triggers ZOA reaper for ${var.cluster_id} every 5 minutes"
  ...
  # Reaper disabled until rosa-boundary integration
  state = "DISABLED"
}
