...
variable "lambda_worker_concurrency" {
  description = "Reserved concurrency for the Worker Lambda. Must be >1 to allow reconciler + concurrent TA executions. Recommended: 10 (1 for reconciler + 9 for TAs)."
  type        = number
  default     = 10
}
...
variable "reconciler_deadline_seconds" {
  description = "Code-level deadline for reconciler/GC/reaper paths. If exceeded, code exits cleanly and defers to next tick. Must be < lambda_worker_timeout."
  type        = number
  default     = 55
}

variable "max_batch_per_tick" {
  description = "Max items processed per reconciler/GC/reaper tick. Remaining items deferred to next tick. Prevents timeout under high load."
  type        = number
  default     = 30
}
